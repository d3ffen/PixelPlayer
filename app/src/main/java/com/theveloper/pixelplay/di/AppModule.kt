...
object AppModule {
    ...
    // ── Album Art Enrichment APIs ─────────────────────────────────────────────

    /**
     * MusicBrainz API — free, no key, requires User-Agent (added by shared OkHttpClient).
     * Rate limit 1 req/s is enforced by the shared client's existing 8-s timeouts.
     */
    @Provides
    @Singleton
    @MusicBrainzRetrofit
    ...
    ): com.theveloper.pixelplay.data.network.musicbrainz.MusicBrainzApiService {
        return retrofit.create(
            com.theveloper.pixelplay.data.network.musicbrainz.MusicBrainzApiService::class.java
        )
    }

    /**
     * Cover Art Archive — free, no key.
     * Responses may redirect (302) — OkHttp follows them automatically.
     */
    @Provides
    @Singleton
    ...
    ): com.theveloper.pixelplay.data.network.coverartarchive.CoverArtArchiveApiService {
        return retrofit.create(
            com.theveloper.pixelplay.data.network.coverartarchive.CoverArtArchiveApiService::class.java
        )
    }

    /**
     * Fanart.tv — requires personal API key in local.properties:
     *   FANART_TV_API_KEY=your_key_here
     * Free for personal use: https://fanart.tv/get-an-api-key/
     */
    @Provides
    @Singleton
    @FanartTvRetrofit
    fun provideFanartTvRetrofit(okHttpClient: OkHttpClient): Retrofit {
        return Retrofit.Builder()
    ...
    @Provides
    @Singleton
    fun provideFanartTvApiService(
        @FanartTvRetrofit retrofit: Retrofit
    ): com.theveloper.pixelplay.data.network.fanart.FanartTvApiService {
        return retrofit.create(
            com.theveloper.pixelplay.data.network.fanart.FanartTvApiService::class.java
        )
    }
}
