...
import com.theveloper.pixelplay.presentation.components.AlbumMetadataInfoSheet
...
fun AlbumDetailScreen(
    ...
    // Write-permission launcher — used on Android 11+ when modifying audio files
    ...
    // Observe write-permission requests from the state holder
    LaunchedEffect(Unit) {
        viewModel.enrichmentStateHolder.writePermissionRequest.collect { intentSender ->
    ...
    // Auto-close sheet on Done or ApplyError after a short delay
    LaunchedEffect(enrichmentState) {
        if (enrichmentState is AlbumEnrichmentState.Done) {
            kotlinx.coroutines.delay(1800)
            showInfoSheet = false
            viewModel.dismissEnrichmentSheet()
        }
    }
    ...
    MaterialTheme(
                ...
                Box(
                    ...
                    if (UseSharedCollapsibleTopBarProbe) {
                        SharedAlbumTopBarProbe(
                            ...
                            onInfoClick = {
                                showInfoSheet = true
                                viewModel.openInfoSheet(album, songs)
                            }
                        )
                    } else {
        ...
        // ── Album Metadata Info Sheet ─────────────────────────────────────────
        if (showInfoSheet && uiState.album != null) {
            AlbumMetadataInfoSheet(
                enrichmentState = enrichmentState,
                onApply = { viewModel.applyEnrichment(uiState.songs) },
                onDismiss = {
                    showInfoSheet = false
                    viewModel.dismissEnrichmentSheet()
                }
...
private fun SharedAlbumTopBarProbe(
    ...
    Box(
        ...
        // ⓘ Info icon — fixed top-end, always visible
        FilledIconButton(
            onClick = onInfoClick,
            modifier = Modifier
            ...
            Icon(
                ...
                contentDescription = "Album info & metadata"
            )
        }
